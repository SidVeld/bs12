...
/mob/proc/show_goals(var/show_success = FALSE, var/allow_modification = FALSE)
	...
	if(LAZYLEN(mind.goals))
		to_chat(src, SPAN_NOTICE("<font size = 3><b>В этой смене вы преследуете следующие личные цели:</b></font><br>[jointext(mind.summarize_goals(show_success, allow_modification, mind.current), "<br>")]"))
	else if(prefs_no_personal_goals)
		to_chat(src, SPAN_NOTICE("<font size = 3><b>Ваши текущие предпочтения не учитывают личных целей.</b></font>"))
	else
		to_chat(src, SPAN_NOTICE("<font size = 3><b>В этой смене у вас нет личных целей.</b></font>"))
	...
	if(LAZYLEN(mind.goals))
		to_chat(mind.current, SPAN_NOTICE("<br><br>Вы можете проверить свои  цели в смене с помощью команды <b>Show Goals</b> в панели IC."))
